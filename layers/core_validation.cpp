...
PHYSICAL_DEVICE_STATE *CoreChecks::GetPhysicalDeviceState(instance_layer_data *instance_data, VkPhysicalDevice phys) {
    auto it = physical_device_map.find(phys);
    if (it == physical_device_map.end()) {
        return nullptr;
    }
    return &it->second;
}

PHYSICAL_DEVICE_STATE *CoreChecks::GetPhysicalDeviceState(const layer_data *device_data) { return physical_device_state; }

PHYSICAL_DEVICE_STATE *CoreChecks::GetPhysicalDeviceState(layer_data *device_data) { return physical_device_state; }
...
void CoreChecks::PostCallRecordCreateDevice(VkPhysicalDevice gpu, const VkDeviceCreateInfo *pCreateInfo,
    ...
    if (GetEnables(core_checks)->gpu_validation) {
        ...
        core_checks->gpu_validation_state.reserve_binding_slot = enabled.gpu_validation_reserve_binding_slot;
...
const CHECK_DISABLED *CoreChecks::GetDisables(layer_data *device_data) { return &device_data->disabled; }

const CHECK_ENABLED *CoreChecks::GetEnables(layer_data *device_data) { return &device_data->enabled; }
...
bool CoreChecks::ValidateStageMasksAgainstQueueCapabilities(layer_data *dev_data, GLOBAL_CB_NODE const *cb_state,
    ...
    auto physical_device_state = GetPhysicalDeviceState(dev_data);
...
bool CoreChecks::PreCallValidateQueuePresentKHR(VkQueue queue, const VkPresentInfoKHR *pPresentInfo) {
    ...
    for (uint32_t i = 0; i < pPresentInfo->swapchainCount; ++i) {
        ...
        if (swapchain_data) {
            ...
            if (!device_data->instance_extensions.vk_khr_android_surface) {
                auto surface_state = GetSurfaceState(device_data, swapchain_data->createInfo.surface);
                auto support_it =
